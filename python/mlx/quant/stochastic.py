...
def simulate_gbm(
    S0: float, mu: float, sigma: float, T: float, steps: int, num_paths: int
) -> mx.array:
    """
...
def black_scholes_greeks(
    ...
    d1 = (mx.log(S_arr / K_arr) + (r_arr + 0.5 * sig_arr**2) * T_arr) / (
        sig_arr * sqrt_T
    )
    ...
    if option_type.lower() == "call":
        ...
        delta = N_d1 - 1.0
        theta = (
            -(S_arr * n_d1 * sig_arr) / (2.0 * sqrt_T)
            + r_arr * K_arr * disc * N_minus_d2
        )
