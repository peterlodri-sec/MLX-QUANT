...
// General (bounds-checked, any K/N) ternary quantized x @ w.T path for
// transpose == true, non-batched weights (see ternary_qmv in
// mlx/backend/metal/kernels/ternary_quantized.h). Used whenever
// ternary_qmv_fast's exact-multiple precondition doesn't hold. Same
// GPU-only / fallback-based-autodiff shape as TernaryQmvFast above.
class TernaryQmv : public Custom {
 public:
  TernaryQmv(
      Stream stream,
      std::function<std::vector<array>(std::vector<array>)> fallback,
  ...
  void eval_cpu(const std::vector<array>& inputs, std::vector<array>& outputs)
      override {
    throw std::runtime_error(
        "[TernaryQmv] GPU-only primitive; mlx::quantized_matmul should "
        "never construct this for a CPU stream.");
  }
  ...
  DEFINE_NAME(TernaryQmv);

  bool is_equivalent(const Primitive& other) const override {
    return group_size_ == static_cast<const TernaryQmv&>(other).group_size_;
  }
...
// Ternary tiled GEMM (large-M matmul) for transpose == true, non-batched
// weights (see ternary_qmm_t in mlx/backend/metal/kernels/
// ternary_quantized.h -- mirrors quantized.h's QuantizedBlockLoader/
// qmm_t_impl/affine_qmm_t minus bias and minus batched-weight support).
// Same GPU-only / fallback-based-autodiff shape as TernaryQmvFast above.
class TernaryQmm : public Custom {
 public:
  TernaryQmm(
      Stream stream,
      std::function<std::vector<array>(std::vector<array>)> fallback,
  ...
  void eval_cpu(const std::vector<array>& inputs, std::vector<array>& outputs)
      override {
    throw std::runtime_error(
        "[TernaryQmm] GPU-only primitive; mlx::quantized_matmul should "
        "never construct this for a CPU stream.");
  }
  ...
  DEFINE_NAME(TernaryQmm);

  bool is_equivalent(const Primitive& other) const override {
    return group_size_ == static_cast<const TernaryQmm&>(other).group_size_;
  }
