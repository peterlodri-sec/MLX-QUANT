...
std::pair<Dtype, QuantizationMode> validate_mode_with_type(
  ...
  if (qmode == QuantizationMode::Affine) {
    ...
    if (out_type.has_value()) {
      ...
      qmode == QuantizationMode::Ternary || qmode == QuantizationMode::Maybe ||
...
array quantized_matmul(
  ...
  if (qmode == QuantizationMode::Maybe && to_stream(s).device == Device::gpu &&
      w.ndim() == 2) {
    auto fallback =
  ...
  if (qmode == QuantizationMode::Mergeq && to_stream(s).device == Device::gpu &&
      w.ndim() == 2) {
    auto fallback =
        [group_size, bits, s, transpose](
            const std::vector<array>& inputs) -> std::vector<array> {
...
array gather_qmm(
  ...
  if (qmode == QuantizationMode::Ternary || qmode == QuantizationMode::Maybe ||
      qmode == QuantizationMode::Mergeq) {
